...
**Vision:** To build a "DevOps-Ready" collaborative grocery application that balances financial control for managers with a robust, observability-first engineering playground, tailored for the Israeli market context ("Israel Budget Guard").
...
## 2. User Personas
* **The "CFO" Manager (Admin/User):** * **Goal:** Control the family budget.
...
* **The "Requestor" Member (User):** * **Goal:** Get snacks (e.g., Bamba, Candy).
...
* **FR-04 (Member View):** Users identified as "Member" MUST ONLY be able to "Request" items. They CANNOT view the running Budget total.
* **FR-05 (Manager View):** Users identified as "Manager" MUST HAVE an "Approval Queue" view to Approve or Reject member requests.
...
* **FR-09:** The Total Budget calculation MUST update immediately upon item addition (for Managers).
